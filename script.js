...
document.querySelectorAll(".btn-touch").forEach(btn => {
    btn.addEventListener("pointerdown", (e) => {
        ...
        e.stopPropagation(); // evita propagación extra

        if(gameOver){
            restartGame(); // reinicia si terminó la partida
            return;
        }
...
btn.addEventListener("pointerup", e => e.preventDefault());
    ...
    btn.addEventListener("click", (e) => {
        if(touched){
            e.stopPropagation();
            e.preventDefault();
            touched = false; // resetea para la siguiente pulsación
        }
    });
});
document.querySelectorAll(".btn-touch").forEach(btn => {
    btn.addEventListener("touchstart", (e) => {
        e.preventDefault(); // evita doble disparo de eventos táctiles
        if(gameOver){
            restartGame();
            return;
        }
